...
When it comes to programming langauges, **loops** basically allow repetitions of some operation. In _Lua_, like in other languages, loops can be represented by `for` and also by `while`.

### For loops

In `for` loops, we specify the value from which the loop begins, the final value (inclusive) and an increment - that is **1** by default.
...
Note that, in this case, the increment could be omitted:
...
> [!NOTE]
> Once `i` is a **local variable**, it cannot be acessed from out of the _for_ loop:
...
In _Lua_, while loops work just the same way we are already familiar with.

 - So we can use then with and explicit condition:
 ...
 - We can also create infinite loops:
...
_Lua_ allows us to use a loop structure by which we ensure at least one execution/repetition.

 ```lua
local x = 11
-- let's try with while first
while x < 10 do
   print("Hey!")
   x = x + 1
end
-- now with repeat-until
repeat
    print("Hey!")
    x = x + 1
until x > 10
...
We will see that `repat-until` allows at least one execution.

```diff
     # nothing for while
Hey! # one repetition for repeat-until
```

But what if we use `false` as the _until_ complement? What will be the ouput?
...
Well, it **will print "hey!" forever!** It happens because, in _Lua_, only `nil` and `false` are _falsy_, which means that this "until condition" will never be true.
